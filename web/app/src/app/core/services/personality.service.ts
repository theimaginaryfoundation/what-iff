...
export class PersonalityService {
  ...
  listPersonalities(page: number = 1, limit: number = 10, filters?: PersonalityFilters): Observable<PaginatedPersonalityResponse> {
    ...
    if (filters?.personality_ids?.length) {
      params = params.set('personality_ids', filters.personality_ids.join(','));
    }
  ...
  upsertExpression(
    ...
    return this.http.put<PersonalityExpression>(
      `${this.apiUrl}/${personalityId}/expressions/${expressionKey}`,
      request
    );
  }
  ...
  /** Sync path when grid already complete (200 + X-Expression-Grid: unchanged). */
  generateDefaultExpressionGrid(
    personalityId: string,
    opts?: { force?: boolean },
  ): Observable<PersonalityExpression[]> {
    let params = new HttpParams();
    ...
    return this.http.post<PersonalityExpression[]>(
      `${this.apiUrl}/${personalityId}/expressions/generate-default-grid`,
      {},
      { params, observe: 'response' },
    ).pipe(
      map(res => {
        if (res.status === 202) {
          throw new Error('Expected synchronous expression list; job was enqueued instead');
        }
        return res.body ?? [];
      }),
    );
  ...
  createPersonality(personality: CreatePersonalityRequest): Observable<Personality> {
    return this.http.post<Personality>(this.apiUrl, personality).pipe(
      tap(() => {
        // Refresh the personality list after creating
        this.listPersonalities().subscribe();
      })
    );
  }
  ...
  updatePersonality(id: string, personality: UpdatePersonalityRequest): Observable<Personality> {
    return this.http.put<Personality>(`${this.apiUrl}/${id}`, personality).pipe(
      tap(() => {
        // Refresh the personality list after updating
        this.listPersonalities().subscribe();
      })
    );
  }
  ...
  deletePersonality(id: string): Observable<void> {
    return this.http.delete<void>(`${this.apiUrl}/${id}`).pipe(
      tap(() => {
        // Refresh the personality list after deleting
        this.listPersonalities().subscribe();
      })
    );
  }
