...
describe('MemoryViewService', () => {
  ...
  beforeEach(() => {
    ...
    memoryApi.patchMemory.mockReturnValue(
      of({
        id: 'm-1',
        content: 'x',
        level: 'global',
    ...
    TestBed.configureTestingModule({
      providers: [provideZonelessChangeDetection(), MemoryViewService, { provide: MemoryService, useValue: memoryApi }],
    });
  ...
  it('loads first page', () => {
    service.load(1);
    expect(memoryApi.getMemories).toHaveBeenCalledWith(1, 100, expect.any(Object));
  ...
  it('ignores a stale list response after filters change', () => {
    const stale = new Subject<{ results: Memory[]; total_count: number; page: number }>();
    const latest = new Subject<{ results: Memory[]; total_count: number; page: number }>();
    memoryApi.getMemories.mockReturnValueOnce(stale).mockReturnValueOnce(latest);

    service.applyFilters({ ...service.filters(), scope: 'all' });
    service.applyFilters({ ...service.filters(), scope: 'user' });
    ...
    expect(service.filters().scope).toBe('user');
    expect(service.memories().map(memory => memory.id)).toEqual(['user-memory']);
