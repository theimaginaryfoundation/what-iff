...
- Data access utilities include chat/message/memory/personality/ritual/file attachment helpers. Admin account backup restore imports a versioned ZIP of user-owned chats, personalities, memories, moods, and agent jobs into an existing target user, preserves source UUIDs for idempotency, clears provider response IDs, and rebuilds memory embeddings. Restores are intentionally section-by-section rather than one large transaction, so partial imports can be retried with the same archive.
