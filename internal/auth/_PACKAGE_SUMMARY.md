...
- **`ValidateSecret` (`secret.go`) gates JWT_SECRET/JWT_REFRESH_SECRET at boot**, called from `cmd/api-server/main.go`'s `init()` (mirroring `datastore.ValidateTokenEncryptionSecret`'s TOKEN_ENCRYPTION_SECRET check right next to it): a missing, under-`MinSecretLen`, or known-placeholder value (anything that has ever shipped as a `docker-compose.yml`/`.env.example` default) is a fatal startup error, not a runtime 500 the first time someone logs in.
  The secrets themselves are still read ad hoc via `os.Getenv` inside `generateAccessToken`/`generateRefreshToken`/`ValidateAccessToken`/`ValidateRefreshToken` — this only adds a redundant fail-fast gate at boot, it does not thread the values through `server.Config`.
