...
- **Explicit-ENV gating (ADR 0x018):** `Config` captures whether `ENV`/`ENVIRONMENT` was explicitly present (`EnvironmentExplicit`, empty string ≠ set) and whether the two conflict (`EnvironmentConflict`).
  `IsExplicitLocalEnv()` is the only gate allowed to enable a non-vendor `LLMBackend` (`mock`/`local`) or `DESTRUCTIVE_MIGRATION` — the parsed `Environment` defaults to `development` when unset, which would be fail-open.
  `cmd/api-server` fatals on conflict, an unrecognized `LLM_BACKEND`, `local` without `LOCAL_LLM_MODEL`, or a non-vendor backend / destructive flag outside an explicit local/test env.
- **LLM backend wiring:** `Config.LLMBackend` (`vendor` default / `mock` / `local`, env `LLM_BACKEND`) plus `LocalLLMBaseURL`/`LocalLLMModel`.
  When non-vendor, `setupRoutes` builds `provider.DenyNetworkHTTPClient()` and threads it into `agent.AgentConfig.HTTPClient`, the memory handler constructor, and the embedder handed to plugins through `plugins.Deps.CreateEmbedding`, so every provider SDK client is egress-denied at the transport (the local backend's own client is constructed separately inside `agent.NewAgent` with real egress).
- **Dependency metrics wiring:** before the datastore is built, `setupRoutes` calls `datastore.InstrumentEntClient` (ent query/mutation durations) and `database.RegisterPoolMetrics` (pool gauges).
  The provider HTTP client is always non-nil now: `telemetry.InstrumentHTTPClient` wraps the deny transport under a non-vendor backend and `http.DefaultTransport` under `vendor` (the SDKs' own default is `http.DefaultClient`, so behaviour is unchanged), recording `http.client.request.duration` per attempt with the vendor from the request host; configured provider base URL overrides are added by `dependencyHosts`.
  Web search gets its own instrumented client that keeps `websearch.DefaultTimeout`, the file store is wrapped with `storage.Instrument`, and a linked email transport with `email.Instrument` (dependency `ses`); the no-op sender is left unwrapped.
- **Metering wiring:** `setupRoutes` calls `metering.New(dataStore, logger)` when it is non-nil.
  The private metering implementation registers `metering.New` via `init()` (reading its own configuration from the environment) and is linked by `cmd/api-server`; when omitted (e.g. the open-source build), `metering.New` is nil and `Agent.NewAgent` falls back to `metering.NoopMeter`.
