...
- **Batch memory delete/patch (partial mode):** when `all_or_none` is false, only `ErrMemoryNotFound` is skipped; other errors abort the batch so operators are not told a row was merely "missing" when the datastore failed.
