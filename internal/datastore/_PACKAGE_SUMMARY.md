...
- **Batch memory delete/patch:** requests cap at `models.MaxMemoryBatchIDs` (100) before any work. In partial (`all_or_none=false`) mode, only `ErrMemoryNotFound` is skipped; other errors abort the batch so operators are not told a row was merely "missing" when the datastore failed.
