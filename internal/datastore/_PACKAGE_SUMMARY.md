...
- **Per-domain files** (each adds methods on `Datastore`):
  ...
  - "What if…" branching: `chat_fork.go` — **`ForkChat`** copies a thread's settings and message prefix (through or just before a message) into a new chat with `forked_from_chat_id` / `forked_from_message_id` lineage in one tx; reuses the parent checkpoint when it is at/before the branch point, otherwise marks the branch `rehydration_state=pending` (`forkNeedsRehydration`); **`ListChatBranches`**, **`GetChatName`** for lineage UI.
...
- `chat_fork_test.go` — branch prefix copy (through/before a message, empty branch), checkpoint reuse vs. rehydration, cross-user/cross-thread isolation, branch listing. Hand-written `chats` fixtures get the lineage columns from `ensureChatForkTestColumns` (`chat_fork_testschema_test.go`), applied by `newTestDatastore` for every suite.
