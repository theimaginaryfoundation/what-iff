...
- **Account-activity audit projection:** audit-row metadata remains encoded with its human-readable message for historical storage compatibility, but `ListAccountActivity` splits valid metadata into `AccountActivityEntry.Metadata`; the `/account/activity` client never parses the persisted encoding.
