...
- **Audit trail:** `auditlog.go` appends rows to Ent `AuditLog` for admin model CRUD, quota bucket **consume** / lazy **refill**, account backup import, and profile **memory ZIP** export/import (best-effort writes; failures only log).
