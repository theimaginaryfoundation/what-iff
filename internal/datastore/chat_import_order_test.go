...
func TestNormalizeImportedMessageTimesPreservesOrder(t *testing.T) {
	base := time.Date(2025, 8, 3, 3, 0, 2, 0, time.UTC)
	...
	require.Equal(t, base.Add(time.Nanosecond), normalized[1].SentAt)
	require.Equal(t, base.Add(2*time.Nanosecond), normalized[2].SentAt)
