...
// sequencedSSEServer streams bodies[N] (raw SSE text) for request N, clamped to the last.
...
func TestXiaomiAdapter_LiveReasoningSeparatesToolRounds(t *testing.T) {
	...
	srv := sequencedSSEServer(t, []string{toolRound, chatCompletionSSE([]string{"got it"}, []string{"done"}, "stop")})
	defer srv.Close()
