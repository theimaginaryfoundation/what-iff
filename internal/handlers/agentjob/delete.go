...
func (h *Handler) DeleteAgentJob(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.provider.DeleteAgentJob(r.Context(), userID, id); err != nil {
		...
		h.logger.Error("failed to delete agent job", zap.String("user_id", userID.String()), zap.String("agent_job_id", id.String()), zap.Error(err))
		handlerutils.RespondWithError(w, h.logger, http.StatusInternalServerError, handlerutils.CodeNotSet, "Failed to delete agent job", err)
		return
	}
