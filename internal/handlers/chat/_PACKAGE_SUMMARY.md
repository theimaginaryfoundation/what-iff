...
- **"What if…" branching (`fork.go`):** `POST /chat/{id}/fork` (`ForkChat`) creates a branch via `Store.ForkChat` and, when the result needs it, calls `BranchAgent.EnqueueBranchRehydration`; `GET /chat/{id}/lineage` (`GetChatLineage`) returns the parent (flagged `deleted` when gone) and direct branches.
...
- `fork_test.go` — fork request validation/defaults/error mapping and rehydration handoff (fake `BranchAgent`); lineage for root, branch, and deleted-parent threads.
